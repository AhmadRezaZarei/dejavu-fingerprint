...
from dejavu.config.settings import (FIELD_FILE_SHA1, FIELD_FINGERPRINTED,
                                    ...
                                    FINGERPRINTS_TABLENAME, SONGS_TABLENAME, FIELD_META)
...
class MySQLDatabase(CommonDatabase):
    ...
    SELECT_META_LESS_SONGS = f"""
        SELECT  `{FIELD_META}`, `{FIELD_SONG_ID}`, `{FIELD_SONGNAME}`, `{FIELD_TOTAL_HASHES}`, `{FIELD_META}`
        FROM `{SONGS_TABLENAME}`
        WHERE `{FIELD_FINGERPRINTED}` = 1 AND `{FIELD_META}` IS NULL;
    """
